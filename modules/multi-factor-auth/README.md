...
A **user-verified passkey login skips the challenge** for that sign-in. A passkey is already a phishing-resistant second factor, so re-challenging adds nothing. This is decided per-login from a signal Auth records at sign-in ([`Authentication::getLoginMethod()`](../auth/passkeys.md)); it is not a trusted-device cookie, so a later password login on the same browser is still challenged. Apps that override `mfa/views/form.php` must adopt the `instanceof FormFragment` snippet (see [Drivers](drivers/#form-fragment-drivers)) for the passkey factor to render on `/mfa` at all.
